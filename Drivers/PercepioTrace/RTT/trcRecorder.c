...
ObjectDataTable objectDataTable = { { 0 } };
...
uint32_t PSFEndianessIdentifier = 0x50534600;

/* Used to interpret the data format */
uint16_t FormatVersion = 0x0002;
...
void vTraceStoreHeader(void);

/* Stores the symbol table on Start */
void vTraceStoreSymbolTable(void);

/* Stores the object table on Start */
void vTraceStoreObjectDataTable(void);

/* Internal function for starting/stopping the recorder. */
void intSetRecorderEnabled(int isEnabled);
...
void psfError(int errCode)
{
	...
	{
		errorCode = errCode;
		vTracePrintF(WarnChn, "Error %%d. Stopped recorder.", errorCode);
