...
/* << EST: additional prototypes to avoid compiler warnings */
...
#ifdef _EXCVT
static const uint8_t ExCvt[] = _EXCVT;	/* Upper conversion table for extended characters */
#endif
...
uint32_t get_fat (	/* 0xFFFFFFFF:Disk error, 1:Internal error, 2..0x0FFFFFFF:Cluster status */
	FATFS* fs,	/* File system object */
	uint32_t clst	/* FAT item index (cluster#) to get the value */
)
{
	UINT wc, bc;
	uint8_t *p;
...
FRESULT put_fat (
	FATFS* fs,	/* File system object */
	uint32_t clst,	/* FAT item index (cluster#) to be set */
	uint32_t val	/* New value to mark the cluster */
)
{
	UINT bc;
...
static
uint8_t check_fs (	/* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
	FATFS* fs,	/* File system object */
